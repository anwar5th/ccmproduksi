                                ...
                                @foreach($machines as $machine)
                                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center bg-slate-50 p-4 rounded-lg border border-slate-200">
                                        ...
                                        <div class="md:col-span-3">
                                            ...
                                            <input type="datetime-local" name="tglm{{ $machine['id'] }}" value="{{ old('tglm'.$machine['id']) ? \Carbon\Carbon::parse(old('tglm'.$machine['id']))->format('Y-m-d\TH:i') : '' }}" class="block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">
                                        </div>
                                        
                                        <div class="md:col-span-3">
                                            <label class="block text-xs font-semibold text-slate-500 mb-1">Tanggal Keluar</label>
                                            <input type="datetime-local" name="tglk{{ $machine['id'] }}" value="{{ old('tglk'.$machine['id']) ? \Carbon\Carbon::parse(old('tglk'.$machine['id']))->format('Y-m-d\TH:i') : '' }}" class="block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">
                                        </div>

                                        <div class="md:col-span-4">
                                            <label class="block text-xs font-semibold text-slate-500 mb-1">Keterangan</label>
                                            <input type="text" name="ket{{ $machine['id'] }}" value="{{ old('ket'.$machine['id']) }}" class="block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm" placeholder="Keterangan opsional...">
                                        </div>
                                    </div>
                                @endforeach
