        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-xl border border-slate-200 overflow-hidden">
                <div class="p-6 sm:p-8">
                    <form action="{{ route('proyekorders.update', $proyekorders->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" id="kodepo" name="kodepo" value="{{ old('kodepo', $proyekorders->kodepo) }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm {{ $errors->has('kodepo') ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : '' }}">
                            @error('kodepo')
                        ...
                        <div>
                            ...
                            <input type="text" id="namaproyek" name="namaproyek" value="{{ old('namaproyek', $proyekorders->namaproyek) }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm {{ $errors->has('namaproyek') ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : '' }}">
                            @error('namaproyek')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" id="tglpo" name="tglpo" value="{{ old('tglpo') ? \Carbon\Carbon::parse(old('tglpo'))->format('Y-m-d\TH:i') : ($proyekorders->tglpo ? \Carbon\Carbon::parse($proyekorders->tglpo)->format('Y-m-d\TH:i') : '') }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm {{ $errors->has('tglpo') ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : '' }}">
                            @error('tglpo')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="keteranganpoitem" name="keteranganpoitem" rows="5" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm {{ $errors->has('keteranganpoitem') ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : '' }}">{{ old('keteranganpoitem', $proyekorders->keteranganpoitem) }}</textarea>
