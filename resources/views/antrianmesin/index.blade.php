        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm rounded-xl border border-slate-200 overflow-hidden">
                    ...
                    <form action="{{ route('antrianmesin.index') }}" method="GET" class="w-full mt-4 sm:mt-0">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-6 gap-2 w-full">
                            ...
                            <div class="sm:col-span-6 flex items-end justify-end gap-2">
                                ...
                                <a href="{{ route('antrianmesin.index', ['perPage' => request('perPage', 10)]) }}" class="inline-flex items-center px-3 py-1.5 bg-slate-100 text-slate-700 hover:bg-slate-200 rounded-md text-sm font-semibold">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full divide-y divide-slate-200 text-xs" style="min-width: 1400px;">
                        <thead class="bg-slate-50">
                            <tr>
                                <th scope="col" class="px-2 py-2 text-left font-semibold text-slate-600 uppercase tracking-wider">Nama PO</th>
                                <th scope="col" class="px-2 py-2 text-left font-semibold text-slate-600 uppercase tracking-wider">No SPK</th>
                                <th scope="col" class="px-2 py-2 text-left font-semibold text-slate-600 uppercase tracking-wider">Tanggal SPK</th>
                                <th scope="col" class="px-2 py-2 text-left font-semibold text-slate-600 uppercase tracking-wider">Deadline</th>
                                <th scope="col" class="px-2 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">Prioritas</th>
                                <th scope="col" class="px-2 py-2 text-left font-semibold text-slate-600 uppercase tracking-wider">Nama Barang</th>
                                <th scope="col" class="px-2 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">Qty</th>

                                <th scope="col" class="px-1 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">Hotpress</th>
                                <th scope="col" class="px-1 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">R.Saw</th>
                                <th scope="col" class="px-1 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">CNC5Ax</th>
                                <th scope="col" class="px-1 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">CNC4Ax</th>
                                <th scope="col" class="px-1 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">Boring</th>
                                <th scope="col" class="px-1 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">Router</th>
                                <th scope="col" class="px-1 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">Rakit</th>
                                <th scope="col" class="px-1 py-2 text-center font-semibold text-slate-600 uppercase tracking-wider">Finish</th>
                                <th scope="col" class="px-2 py-2 text-right font-semibold text-slate-600 uppercase tracking-wider">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-slate-200">
                            @forelse ($antrianmesin as $antri)
                                <tr class="hover:bg-slate-50 transition-colors duration-150">
                                    <td class="px-2 py-2.5 whitespace-nowrap font-medium text-slate-900">
                                        {{ $antri->proyekorder->namaproyek }}
                                    </td>
                                    <td class="px-2 py-2.5 whitespace-nowrap text-slate-600">
                                        {{ $antri->nospk }}
                                    </td>
                                    <td class="px-2 py-2.5 whitespace-nowrap text-slate-600">
                                    ...
                                    <td class="px-2 py-2.5 whitespace-nowrap text-slate-600">
                                        {{ $antri->deadline ? \Carbon\Carbon::parse($antri->deadline)->format('d M Y') : '-' }}
                                    </td>
                                    <td class="px-2 py-2.5 whitespace-nowrap text-center">
                                        @if($antri->deadline && \Carbon\Carbon::parse($antri->deadline)->startOfDay()->lte(now()->addDays(2)->startOfDay()))
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-bold {{ $antri->prioritas == 1 ? 'bg-red-100 text-red-700 border border-red-200' : ($antri->prioritas ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-slate-100 text-slate-600') }}">
                                                {{ $antri->prioritas ?? '-' }}
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-2 py-2.5 whitespace-nowrap text-slate-900 truncate max-w-[120px]" title="{{ $antri->namabarang }}">
                                        {{ $antri->namabarang }}
                                    </td>
                                    <td class="px-2 py-2.5 whitespace-nowrap text-center">
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-bold {{ $antri->qtybarang <= 10 ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-emerald-100 text-emerald-700 border border-emerald-200' }}">
                                            {{ $antri->qtybarang }}
                                        </span>
                                    </td>
 
                                    <!-- Hotpress -->
                                    <td class="px-1 py-2.5 whitespace-nowrap text-center">
                                        @if ($antri->tglkhotpress)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-100 text-emerald-800 border border-emerald-200">Selesai</span>
                                        @elseif ($antri->tglmhotpress)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-100 text-amber-800 border border-amber-200 shadow-sm animate-pulse">Proses</span>
                                        @else
                                            <span class="inline-flex items-center px-1 rounded text-[10px] font-medium text-slate-400">-</span>
                                        @endif
                                    </td>
 
                                    <!-- Running Saw -->
                                    <td class="px-1 py-2.5 whitespace-nowrap text-center">
                                        @if ($antri->tglkrunningsaw)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-100 text-emerald-800 border border-emerald-200">Selesai</span>
                                        @elseif ($antri->tglmrunningsaw)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-100 text-amber-800 border border-amber-200 shadow-sm animate-pulse">Proses</span>
                                        @else
                                            <span class="inline-flex items-center px-1 rounded text-[10px] font-medium text-slate-400">-</span>
                                        @endif
                                    ...
                                    <!-- CNC 5 Axis -->
                                    <td class="px-1 py-2.5 whitespace-nowrap text-center">
                                        @if ($antri->tglkcnc5axis)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-100 text-emerald-800 border border-emerald-200">Selesai</span>
                                        @elseif ($antri->tglmcnc5axis)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-100 text-amber-800 border border-amber-200 shadow-sm animate-pulse">Proses</span>
                                        @else
                                            <span class="inline-flex items-center px-1 rounded text-[10px] font-medium text-slate-400">-</span>
                                        @endif
                                    ...
                                    <!-- CNC 4 Axis -->
                                    <td class="px-1 py-2.5 whitespace-nowrap text-center">
                                        @if ($antri->tglkcnc4axis)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-100 text-emerald-800 border border-emerald-200">Selesai</span>
                                        @elseif ($antri->tglmcnc4axis)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-100 text-amber-800 border border-amber-200 shadow-sm animate-pulse">Proses</span>
                                        @else
                                            <span class="inline-flex items-center px-1 rounded text-[10px] font-medium text-slate-400">-</span>
                                        @endif
                                    ...
                                    <td class="px-2 py-2.5 whitespace-nowrap text-right font-medium">
                                        <a href="{{ route('antrianmesin.show', $antri->id) }}" class="inline-flex items-center px-2 py-1 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 rounded transition-colors text-[10px] font-semibold shadow-sm">
                            ...
                            @empty
                                <tr>
                                    <td colspan="16" class="px-6 py-8 text-center text-slate-500 bg-slate-50">
                ...
                <div class="px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-col lg:flex-row items-center justify-end gap-4">
                    <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-center lg:justify-start">
                        <form action="{{ route('antrianmesin.index') }}" method="GET" id="perPageForm" class="flex items-center gap-2">
                        ...
                        <form action="{{ route('antrianmesin.index') }}" method="GET" id="pageJumpForm" class="flex items-center gap-2">
                            <input type="hidden" name="po" value="{{ request('po') }}">
                            <input type="hidden" name="nospk" value="{{ request('nospk') }}">
                            <input type="hidden" name="namabarang" value="{{ request('namabarang') }}">
                            <input type="hidden" name="tglspk_from" value="{{ request('tglspk_from') }}">
